...
> Gobin v1.4.0 起随 `--jobs` 发布。本指南说明 `--jobs` 的取值含义、产物一致性边界，以及怎么挑一个合适的工作并发数。
...
- 启 N 个 worker，把 `PageSpec` 列表按索引 stripe 拆给 worker；
- 每个 worker 独立渲染自己那份页面并 `os.WriteFile`；
- worker 内 local 统计在末尾合并，首个错误经原子标志位让其它 worker 尽快停下。

**不在并行范围**内（保持串行）：

- Markdown 解析（`parser.ParsePosts` / `ParsePages`）；
- feed / atom / sitemap / search index / aliases / robots 等聚合产物；
- 静态资源复制；
- `cleanOutput` 的目录清理；
- 写 `buildManifest`。

原因是这些阶段要么是 I/O 不密集（解析）、要么是单文件单写者（manifest）、要么强依赖全局顺序（聚合）。把它们一起并行收益小、风险大。
...
| 100–1000 篇 | `0`（默认）| 4 worker 在多核机器上拿 15–19% 收益；自动封顶避坑 |
...
`go test -race ./...` 已经覆盖了并行渲染路径（`internal/generator/parallel_test.go` + `incremental_bench_test.go`）。如果你的 fork 改了渲染逻辑导致并发问题，最先怀疑的点：
