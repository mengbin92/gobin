...
> Gobin v1.4.0 起把诊断日志统一到 `log/slog`。本指南说明 stdout / stderr / 文件分别承担什么、`--verbose` / `--log-format` / `--log-file` 怎么用、CI 怎么把日志和用户输出分离。
...
| 诊断日志 | DEBUG / INFO / WARN / ERROR 级别的结构化日志 | **stderr**（或 `--log-file`） |
...
| `--log-file` | 文件路径 | 空（→ stderr）| 把诊断日志追加写文件；路径不可写则回退 stderr 并提示 |
...
| `version` / `commit` / `build_date` | `commands` 层启动事件 | `version=v1.4.0` |
...
`--log-file` 是**追加**写，路径不可写会**回退**到 stderr 并打一条 WARN，不让构建崩。

## 6. 内部可观测性（`component` 标签）
...
| `--log-file /readonly/x.log` 报权限错 | 文件不可写 | 修路径；不可写时 Gobin 会自动回退 stderr 并打 WARN，不让构建崩 |
| 模板里 `{{ .Site.Logger }}` 找不到 | 模板上下文没有 logger 字段 | 是设计如此；Gobin 不把 logger 暴露给模板，让模板无副作用 |
