...
func GenerateRSSFeed(posts []*parser.Post, cfg *config.Config, outputDir string) error {
	...
	feed := RSSFeed{
		...
		Channel: RSSChannel{
			...
			Generator:     "gobin",
			AtomLink: AtomLink{
				Href: fmt.Sprintf("%s/index.xml", cfg.BaseURL),
				Rel:  "self",
	...
	for _, post := range posts[:maxPosts] {
		item := RSSItem{
			Title:       post.Title,
			Link:        fmt.Sprintf("%s%s", cfg.BaseURL, post.URL),
			Description: post.Summary,
			Author:      cfg.Author,
			PubDate:     post.Date.UTC().Format(time.RFC1123Z),
			GUID: GUID{
				Value:       fmt.Sprintf("%s%s", cfg.BaseURL, post.URL),
				IsPermaLink: "true",
			},
...
func GenerateAtomFeed(posts []*parser.Post, cfg *config.Config, outputDir string) error {
	...
	feed := AtomFeed{
		...
		Link: []Link{
			...
			{
				Href: fmt.Sprintf("%s/index.atom", cfg.BaseURL),
				Rel:  "self",
	...
	for _, post := range posts[:maxPosts] {
		entry := AtomEntry{
			Title:   post.Title,
			Link: Link{
				Href: fmt.Sprintf("%s%s", cfg.BaseURL, post.URL),
				Rel:  "alternate",
			},
			ID:        fmt.Sprintf("%s%s", cfg.BaseURL, post.URL),
